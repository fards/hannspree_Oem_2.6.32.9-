...
static int debug =0;//DEBUG_TRACE;//DEBUG_TRACE;
static int comms = 0;
...
static void report_sync(struct mxt_data *mxt)
{
	...
	int fin = 0;
	   ...
	   for (i = 0; i < MAX_FINGERS; i++) {
		if (fingers[i].size != 0) {
			fin++;
			mxt_debug(DEBUG_TRACE, "report_sync touch_number=%d, x=%d, y=%d, width=%d\n",
	 ...
	 if (fin == 0){
		mxt_debug(DEBUG_TRACE, "report_sync no fingler\n");
...
#if 0
#ifdef CONFIG_ABS_MT_TRACKING_ID
...
void process_T9_message(u8 *message, struct mxt_data *mxt)
{
	...
	mxt_debug(DEBUG_TRACE,  " ----------------process_T9_message ------------ \n");
	...
	/*******debug print*******/
	//printk("Status =%x,Report ID=%x,TouchNumber=%d \n", status,report_id,touch_number);
	
	if (status & MXT_MSGB_T9_SUPPRESS) {
		/* Touch has been suppressed by grip/face */ 
		/* detection                              */
		mxt_debug(DEBUG_TRACE, "SUPRESS");
		report_mt(touch_number, 0, 0, 0, mxt);
		report_sync(mxt);
	} else {
		mxt_debug(DEBUG_TRACE, "XMLB = %d, YMLB = %d, XYLSB = %d\n", 
				message[MXT_MSG_T9_XPOSMSB], 
				message[MXT_MSG_T9_YPOSMSB], 
				message[MXT_MSG_T9_XYPOSLSB]);
		...
		//touch_number = message[MXT_MSG_REPORTID] - //delete at 110302 by jone
		//	mxt->rid_map[report_id].first_rid;
           //   malata_touch_number=touch_number;
		if (status & MXT_MSGB_T9_DETECT) {
			 ...
			 */
			mxt_debug(DEBUG_TRACE,  " ----process_T9_message --MXT_MSGB_T9_DETECT--- \n");
			...
			mxt_debug(DEBUG_TRACE, " ----process_T9_message --MXT_MSGB_T9_RELEASE--- \n");
...
static void mxt_worker(struct work_struct *work)
{
	...
	mxt_debug(DEBUG_TRACE,  " --mxt_worker  -----comms is (%d)--report_id is (%d)----- \n", 
			comms,report_id);	
	//do {
		mxt_debug(DEBUG_TRACE, " --mxt_worker  do ----report_id is (%d)-------- \n", report_id);
      ...
      // printk("message_addr=%d,message_length=%d,message[0]=%d\n",message_addr,message_length,message[0]);
		if (mxt->address_pointer != message_addr)
			mxt->valid_ap = 0;
		if(touch_pannel_no_join==0) // resolve no connect touch pannel will dead at machine pwr on problem 
		{
		 	if((message[0]>5)&&(message[0]<255))
		  	{
                   		report_id=255;
 		  	}
		  	else
	...
	//} while ((report_id != MXT_END_OF_MESSAGES) && (report_id != 0));//lu delete  it and now is one touch point
	//occur one irq
	kfree(message);
...
static irqreturn_t mxt_irq_handler(int irq, void *_mxt)
{
	...
	mxt_debug(DEBUG_TRACE, "mxt_irq_handler() IN\n");
			       ...
			       const struct i2c_device_id *id)
{
	...
	cdev_init(&mxt->cdev_messages, &mxt_message_fops);
	 mxt_write_byte(mxt->client,MXT_BASE_ADDR(MXT_TOUCH_MULTITOUCHSCREEN_T9, mxt)+9,7);//touch pannel rotate lu modify
        mxt_write_byte(mxt->client,MXT_BASE_ADDR(41, mxt)+0,1);//touch pannel paml press dead lu modify 0
        mxt_write_byte(mxt->client,MXT_BASE_ADDR(41, mxt)+3,40);//touch pannel paml press dead lu modify 0
        mxt_write_byte(mxt->client,MXT_BASE_ADDR(41, mxt)+5,5);//touch pannel paml press dead lu modify 0
        mxt_write_byte(mxt->client,MXT_BASE_ADDR(MXT_TOUCH_MULTITOUCHSCREEN_T9, mxt)+14,MAX_FINGERS);//touch number set to 4
       mxt_write_byte(mxt->client,MXT_BASE_ADDR(MXT_GEN_COMMANDPROCESSOR_T6, mxt)+MXT_ADR_T6_BACKUPNV,MXT_CMD_T6_BACKUP);//touch pannel rotate lu modify
...
/*
	{
		...
		int address = MXT_BASE_ADDR(MXT_TOUCH_MULTITOUCHSCREEN_T9, mxt) + 9;
		mxt_read_block(mxt->client, address, 1, buffer);
		mxt_debug(DEBUG_TRACE, "Byte 9= %d\n", buffer[0]);
		address = MXT_BASE_ADDR(MXT_TOUCH_MULTITOUCHSCREEN_T9, mxt) + 18;
		mxt_read_block(mxt->client, address, 4, buffer);
		for (i = 0; i < 4; i++) {
			mxt_debug(DEBUG_TRACE, "Byte %d = 0X%0x\n", i+18, buffer[i]);
		}
	}*/
	return 0;
...
static int __init mxt_init(void)
{
	...
	if (err) {
		...
		printk( "Adding maXTouch driver fail \n");
	} else {
		mxt_debug(DEBUG_TRACE, "Successfully added driver %s\n",
		          mxt_driver.driver.name);
		printk( "Adding maXTouch driver  success \n");
	}
